...
=M-x doclive-open-wiki= でWikiのルートディレクトリを指定します。
Markdown文書を置いた通常のフォルダをルートとして使えます。
次回からは最近開いたルートを補完候補から選べます。
ルートのREADMEを開き、READMEがなければパス順で最初の文書を開きます。
Emacsで訪問するのは選択したページだけです。

Libraryサイドバーでフォルダをたどるか、Search all pages欄でWiki全体のタイトル、パス、本文を検索します。
ツールバーのLibraryボタンでサイドバーを開閉できます。
タイトルやパスの断片を空白で区切ると、順序を問わず検索できます。
たとえば =ref sto= で =reference/storage.md= が見つかります。
本文では入力全体をそのまま連続一致で検索します。
タイトルやパスの一致を本文だけの一致より先に表示し、結果は最大100件です。
上限を超える結果を見るには検索語を絞り込んでください。結果の次ページはありません。
サイドバーのSavedタブにはブックマーク、Recentには閲覧したページが表示されます。
本文上のSave pageでブックマークすると、そのボタンがSavedに変わり、再度押すと解除します。文書内容の保存とは別の操作です。
ツールバーのFind in pageは現在の文書を検索します。その隣のPinは、別ページに移動しても検索語のハイライトを保持します。
ピン留めした検索語はページ固有ではない共通の閲覧設定で、別のWikiを開いたときも復元します。
検索語のチップにある×で解除できます。ページのブックマークとは異なります。
ツールバーのDark/Lightでテーマを、A-・A・A+で文字サイズを変更できます。
Wiki内の遷移はシンボリックリンクの解決後も指定ルート内に限定されます。
ディレクトリへのシンボリックリンクと =.git=、=.svn=、=node_modules=、=.cache= は索引から除外します。

最近開いたルート、閲覧履歴、ブックマーク、テーマ、倍率、検索ピンは =doclive-wiki-state-file= に保存します。
既定の保存先はEmacsのユーザーディレクトリ内の =doclive-wiki.json= です。
=M-x customize-option RET doclive-wiki-state-file RET= でこのオプションを変更できます。
=nil= にすると、設定の保持は現在のEmacsセッション内だけになります。
保存ファイルにはローカルのWikiパスを含むため、文書リポジトリにはコミットしないでください。
...
fragment や query はファイル解決時には無視され、remote URL や未対応拡張子はローカル文書として開きません。HTTP(S)リンクはブラウザのリンクとして扱います。
既定では、シンボリックリンク解決後の実パスで判定し、リンク先をクリック元ファイルのディレクトリ配下に限定します。
=../notes.md= のように親ディレクトリへ出るリンクを許可したい場合は、信頼できる文書だけで =doclive-allow-linked-document-parent-directory= を有効にしてください。
Wikiモードでは指定したWikiルートが境界となり、このオプションを有効にしてもルート外には移動できません。
...
doclive はグローバルなキーバインドを追加しません。起動コマンドを好みのキーに割り当ててください。
...
次のキーはプレビューバッファ内だけで有効です。

- =C-s= または =s-f= : Emacs のミニバッファからページ内を検索。検索中の =C-s= で次、=C-r= で前の一致へ移動します。
- =s-k= : ミニバッファから Wiki のファイル名と本文を検索します。
- =Escape= : 検索、サイドバー、拡大した図を閉じます。
- =M-<left>= / =M-<right>= : 閲覧履歴を戻る / 進む。

- =kill-ring-save=（=M-w=）と macOS の =ns-copy-including-secondary=（=s-c=）はページ内で選択中のテキストを kill ring に入れます。
- =yank=（=C-y=, =s-v=）はフォーカス中のテキスト入力欄へ kill ring の先頭を貼り付けます。Wiki 検索欄にも使えます。入力欄にフォーカスがなければ、ページ内検索ボックスに入れて一致をハイライトします。

コードブロックの Copy ボタンも Emacs の kill ring に送ります。
文字入力と編集キーは、フォーカス中の HTML 要素へ直接渡します。=xwidget-webkit-edit-mode= への切替は不要です。
=C-x= と =M-x= は引き続き Emacs のコマンドとして使えます。
...
静的ファイルを配信するルートはありません。Cookie認可された =/workspace= と =/search= はWiki文書の一覧と検索、=/open= は文書を開く操作、=/content= と =/events= はプレビュー更新を担当します。=/copy= はプレビューのテキストをEmacsのkill ringに送ります。文書へのアクセス範囲はローカルリンク遷移の節のとおり制限されます。
...
- =org-preview-html= :: 変更ごとに Org を静的ファイルへ export します。doclive は更新番号を通知し、ブラウザがメモリ上のスナップショット全体を取得します。中間HTMLファイルは作りません。
...
Chromiumの同梱はLinuxのみです。macOSではChromeまたはChromiumを別途導入してください。
...
- *相対リンクが開かない* :: remote URL、=.el=、=.html=、=.pdf= はローカルリンク遷移の対象外です。クリック元ディレクトリの外にあるリンク先も既定では拒否されます。Wikiモードでは指定したルートが境界です。
- *サーバーが起動しない* :: token 生成には =/dev/urandom= か =openssl= が必要です。どちらもない環境（=PATH= に openssl がない素の Windows など）では起動に失敗します。
- *大きな Mermaid 図が読みにくい* :: 図の Fit または 100% 操作でその場のサイズを調整するか、Expand で全画面オーバーレイを開いてマウスホイールでのズームとドラッグでのパンを使ってください。オーバーレイは閉じるボタン、背景クリック、Escape で閉じます。Escape は xwidget プレビューでも使えます。
