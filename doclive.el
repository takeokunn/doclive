...
runtime script and style when it is safe for CSP nonce use."
  (concat
   ...
   "function sanitizeUrlValue(value,allowMailto=false){const raw=String(value==null?'':value); const folded=raw.replace(/[\\u0000-\\u001F\\u007F\\s]+/g,'').toLowerCase(); if(!folded||folded.startsWith('//')||raw.indexOf(String.fromCharCode(92))!==-1) return ''; if(/[\\u0000-\\u001F\\u007F]/.test(raw)) return ''; if(/^[a-z][a-z0-9+.-]*:/.test(folded)&&!(/^https?:/.test(folded)||(allowMailto&&folded.startsWith('mailto:')))) return ''; return raw.trim();}"
