...
inline runtime script when it is safe for CSP nonce use."
  (concat
   ...
   "function sanitizeUrlValue(value){const raw=String(value==null?'':value); const folded=raw.replace(/[\\u0000-\\u001F\\u007F\\s]+/g,'').toLowerCase(); if(!folded||folded.startsWith('//')||raw.indexOf(String.fromCharCode(92))!==-1) return ''; if(/^[a-z][a-z0-9+.-]*:/.test(folded)&&!/^(https?:|mailto:)/.test(folded)) return ''; return raw;}"
