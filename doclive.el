...
inline runtime script when it is safe for CSP nonce use."
  (concat
   ...
   "function sanitizeUrlValue(value){const raw=String(value==null?'':value); const folded=raw.replace(/[\\u0000-\\u001F\\u007F\\s]+/g,'').toLowerCase(); if(!folded||folded.startsWith('//')||raw.indexOf(String.fromCharCode(92))!==-1) return ''; if(/[\\u0000-\\u001F\\u007F]/.test(raw)) return ''; if(/^[a-z][a-z0-9+.-]*:/.test(folded)&&!/^(https?:|mailto:)/.test(folded)) return ''; return raw.trim();}"
  "function sanitizeHtml(html){const tpl=document.createElement('template'); tpl.innerHTML=html||''; tpl.content.querySelectorAll('script,iframe,object,embed,link,meta,base').forEach((el)=>el.remove()); const walker=document.createTreeWalker(tpl.content,NodeFilter.SHOW_ELEMENT); const nodes=[]; while(walker.nextNode()) nodes.push(walker.currentNode); nodes.forEach((el)=>{Array.from(el.attributes).forEach((attr)=>{const name=attr.name||''; const lower=name.toLowerCase(); if(/^on/i.test(name)||lower==='style'||lower==='srcset'){el.removeAttribute(attr.name); return;} if(/^(href|src|xlink:href|formaction|action|poster)$/i.test(name)){const safe=sanitizeUrlValue(attr.value||''); if(safe){el.setAttribute(attr.name,safe);}else{el.removeAttribute(attr.name);}}}); if(el.tagName&&el.tagName.toLowerCase()==='a'&&(el.getAttribute('target')||'').toLowerCase()==='_blank'){el.setAttribute('rel','noopener noreferrer');}}); return tpl.innerHTML;}"
   ...
   "function wireDocumentLinkNavigation(){mdEl.querySelectorAll('a[href]').forEach(a=>{const href=a.getAttribute('href')||''; if(/^[a-zA-Z][a-zA-Z0-9+.-]*:/i.test(href)||href.startsWith('#')||href.startsWith('//')) return; if(!/\\.(md|org|html)($|#|\\?)/i.test(href)) return; a.addEventListener('click',ev=>{ev.preventDefault(); openLinkedDocument(href);});});}"
