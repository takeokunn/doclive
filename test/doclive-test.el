...
(ert-deftest doclive-test-release-docs-match-security-model ()
  ...
  (let ((security (doclive-test--file-string "SECURITY.md"))
    ...
    (should (string-match-p (regexp-quote "intentionally omits `Secure`") security))
    (should (string-match-p (regexp-quote "loopback-only by default") security))
    ...
    (should (string-match-p (regexp-quote "opaque per-buffer random routing identifiers") security))
    (should (string-match-p (regexp-quote "not derived from local file paths or buffer names") security))
    ...
    (should (string-match-p (regexp-quote "not local path or buffer-name") melpa-notes))
    (should (string-match-p (regexp-quote "derivations") melpa-notes))
    ...
    (should (string-match-p (regexp-quote "opaque per-buffer random routing identifiers") news))
    (should (string-match-p (regexp-quote "not local path or buffer-name") news))
    (should (string-match-p (regexp-quote "derivations") news))
...
(ert-deftest doclive-test-valid-query-rejects-ambiguous-or-token-keys ()
  ...
  (should-not (doclive--valid-query-p "/content?id=abc&"))
  (should-not (doclive--valid-query-p "/content?&id=abc"))
  (should-not (doclive--valid-query-p "/content?id=abc&&path=target.md"))
