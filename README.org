...
=file:= / =ftp:= / =javascript:= / =vbscript:= / =data:= / =//= から始まる protocol-relative URL は拒否され、Content-Security-Policy は設定された資源URLの origin から生成されます。
...
Nix を使う場合は、compile / test / lint / package-lint を flake 経由でも確認できます。
PR や release 前には autoload 差分と whitespace も確認するため、最終ゲートとして =make check= も実行してください。
