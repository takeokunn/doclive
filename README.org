...
自動ゲートは =make check= / =pre-commit run --all-files= / =nix flake check= で確認し、
ブラウザ表示の smoke は下記コマンドでサーバ起動経路を検証します。

#+begin_src sh
emacs -Q --batch -L . -l doclive.el \
  --eval '(setq doclive-open-browser-function (lambda (_url) (princ "preview opened\n")))' \
  --eval '(doclive-preview-file (expand-file-name "example/sample.md" default-directory))' \
  --eval '(dotimes (_ 3) (accept-process-output nil 1))' \
  --eval '(doclive-stop-server)' \
  --eval '(kill-emacs)'
#+end_src
